...
@section('content')
    ...
    <nav class="mt-4 mb-2 text-sm text-gray-500 dark:text-gray-400" aria-label="Breadcrumb">
        <ol class="flex items-center gap-2">
            ...
            <li>
                <span class="text-gray-700 dark:text-gray-200">Editar Producto</span>
            </li>
        </ol>
    </nav>
    <div class="container px-6 mx-auto grid">
            ...
            <div class="relative p-6 sm:p-8">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-white/80 text-sm">Modifica los datos del producto.</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <a href="{{ route('products.index') }}" class="flex items-center px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-white/10 hover:bg-white/15 rounded-lg backdrop-blur">
        ...
        <div x-data='{
            editProduct: {
                ...
                category_id: @json(optional($product->brand)->category_id),
                brand_id: @json($product->brand_id),
                unit_measure_id: @json($product->unit_measure_id),
                entity_id: @json($product->entity_id),
                tax_id: @json($product->tax_id),
            ...
            <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data">
                ...
                @method('PUT')
                @php($alpine = true)
                @include('admin.products.form', ['alpine' => true, 'product' => $product])
            </form>
        </div>
    </div>
@endsection
