<div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
    ...
    <label class="block mt-4 text-sm w-full">
        ...
        <div class="mt-2">
            <img id="avatarPreview" src="{{ isset($user) && $user->profile && $user->profile->avatar ? asset('storage/' . $user->profile->avatar) : '' }}" alt="Vista previa" width="80" class="rounded" style="display: {{ isset($user) && $user->profile && $user->profile->avatar ? 'block' : 'none' }};">
        </div>
        @error('avatar')
            <span class="text-xs text-red-600 dark:text-red-400">{{ $message }}</span>
        @enderror
