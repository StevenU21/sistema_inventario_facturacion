@php
    $oldItems = old('items', []);
@endphp
<div x-data="quotationForm()" x-init="init()"
    class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800 border border-gray-200 dark:border-gray-700 w-full">

    <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-3">Datos de la cotización</h3>
            ...
            <div class="md:col-span-9 col-span-1 flex flex-col">
                <label class="block text-sm w-full">
                    ...
                    <x-autocomplete id="client_search" name="client_search" :value="old('client_search')"
                        url="{{ route('entities.clientSearch') }}" placeholder="Nombre, cédula, teléfono..."
    ...
    <input type="hidden" name="entity_id" :value="quotation.entity_id">
    @error('entity_id')
            ...
            <div class="md:col-span-9 col-span-1 flex flex-col">
                <label class="block text-sm w-full">
                    ...
                    <x-autocomplete name="variant_search" id="variant_search"
                        url="{{ route('admin.quotations.productSearch') }}" placeholder="Nombre del producto..."
    ...
    <div class="mt-6">
        <h4 class="text-base font-semibold text-gray-800 dark:text-gray-100">Detalle de la cotización</h4>
        <div class="mt-3 overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                <thead class="bg-gray-50 dark:bg-gray-800/50">
                    <tr class="text-left text-gray-600 dark:text-gray-300">
            ...
            return {
                ...
                init() {
                    ...
                    window.addEventListener('warehouse-changed', (e) => {
                        this.selectedWarehouseId = Number(e.detail) || null;
                    });
                    this.$nextTick(() => {
                        const input = document.querySelector('#client_search');
                        if (input && !input.value && this.quotation.entity_id) {
                            @if (!empty($entities))
                                const opts = @json($entities);
                                const name = opts[String(this.quotation.entity_id)] || '';
                                if (name) input.value = name;
                            @endif
                        }
                        if (this.quotation.entity_id) {
                            window.dispatchEvent(new CustomEvent('selected-entity', {
                                detail: this.quotation.entity_id
                            }));
                        }
                    });
                    window.addEventListener('add-item', (e) => {
                        ...
                        if (!variantId) return;
                        // Validación de almacén: lo manejamos con fetchInventory similar a ventas
                        this.fetchInventory(variantId, rowWarehouseId);
                    });
                    window.addEventListener('client-selected', (e) => {
                        const id = e.detail?.item?.id ?? e.detail?.item?.value ?? null;
                        const text = e.detail?.text ?? '';
                        if (id) {
                            this.quotation.entity_id = Number(id);
                            const input = document.querySelector('#client_search');
                ...
                async fetchInventory(variantId, warehouseId) {
                    try {
                ...
                async saveClient() {
                    ...
                    try {
                        ...
                        if (res.status === 201) {
                            const data = await res.json();
                            this.quotation.entity_id = data.id;
                            const input = document.querySelector('#client_search');
        ...
        function variantSearch() {
            return {
                ...
                async search(page = 1) {
                    this.loading = true;
                    try {
                        const url = new URL(@json(route('admin.quotations.productSearch')));
            ...
            return {
                ...
                init() {
                    ...
                    window.addEventListener('client-added', (e) => {
                        ...
                        const displayName = (d.text || [d.first_name, d.last_name].filter(Boolean).join(' '))
                            .trim() || 'Nuevo cliente';
                        const row = {
                            id: d.id,
                            name: displayName,
    ...
    <x-modal maxWidth="xl" title="Nuevo cliente" description="Cree rápidamente un cliente con los datos mínimos">
        ...
        <div class="mt-4 flex justify-end gap-2">
            ...
            <button type="button" @click="saveClient()"
                ...
                <span class="mr-2 flex items-center"><i class="fas fa-paper-plane"></i></span>
