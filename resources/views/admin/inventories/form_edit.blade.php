        ...
        <div class="flex flex-col md:flex-row gap-4">
            <label class="block text-sm w-full">
                ...
                <div class="relative text-gray-500 focus-within:text-purple-600 dark:focus-within:text-purple-400">
                    <select name="product_variant_id" required class="block w-full pl-10 mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:shadow-outline-purple dark:focus:shadow-outline-gray @error('product_variant_id') border-red-600 @enderror">
                        <option value="">Seleccione</option>
                        @foreach ($variants as $id => $name)
                            <option value="{{ $id }}" {{ (old('product_variant_id', $inventory->product_variant_id ?? null) == $id) ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
